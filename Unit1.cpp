...
int Size;
int p;               	//эталонная вероятность кол. попаданий значений в интервал
double chiSquared;		//хи-квадрат

n_rasp = 50;			//кол. интервалов
chiSquared = 0;
max=Buffer[0];
min=Buffer[0];
Size = Buffer.size();
...
if(test > 0.2)
	st2 = " Проверка  равномерного распределения: тест не пройден ";
else
	st2 = " Проверка  равномерного распределения: тест пройден ";
